...
// Null on the surfaces that are not a mode (the inbox), so their own row is the
// only one lit.
const modeForPath = (pathname: string): AppMode | null =>
...
export function SidebarNav({ className }: { className?: string }) {
  ...
  return (
    <nav className={cn("flex flex-col gap-0.5 p-2", className)}>
      <Link to="/inbox" className={rowClass(pathname.startsWith("/inbox"))}>
        <IconInbox className="size-4 shrink-0" />
        <span className="truncate">Inbox</span>
        {UNREAD_COUNT > 0 && (
          <span className="ml-auto tabular-nums">{UNREAD_COUNT}</span>
        )}
      </Link>
      {groups.map((group, index) => (
        <div
          key={group[0]?.mode}
          className={cn("flex flex-col gap-0.5", index > 0 && "mt-2")}
        >
          {group.map(({ mode: m, to, label, icon: Icon }) => (
            <Link key={m} to={to} className={rowClass(m === mode)}>
