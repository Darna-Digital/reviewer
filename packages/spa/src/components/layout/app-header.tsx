...
export function AppHeader({ route }: { route: ShellRoute }) {
  ...
  return (
    <header className="flex h-9 shrink-0 items-center gap-2 px-2">
      {/* The project chip leads the window bar, above everything: it names what
          the whole window is on. The branch is where in it you are, which is a
          fact about this page, so it stays down here at the head of the row. */}
      {repo.data != null && (
        <BranchSwitcher
