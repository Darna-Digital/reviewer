/**
 * The small marks that carry an issue's state in a list: its status ring and
 * its priority bars.
 *
...
export function LabelChip({
  ...
  return (
    <span
      className={cn(
        "inline-flex h-5 shrink-0 items-center gap-1.5 rounded-full border border-border/70 pr-2 pl-1.5 text-[11px] whitespace-nowrap text-muted-foreground",
        className
      )}
    >
      <span className={cn("size-2 rounded-full", ACCENT_DOT[color])} />
      {name}
    </span>
  )
}
...
export function ProjectGlyph({
  ...
  return (
    <span
      className={cn(
        "flex size-4 shrink-0 items-center justify-center rounded-[5px] text-[9px] font-semibold text-white uppercase",
        ACCENT_DOT[color],
        className
      )}
      aria-hidden
