...
describe("initialWindowTabs", () => {
  it("opens on the pinned tabs, showing Code", () => {
    expect(show(initialWindowTabs())).toBe("*code git sessions");
  });
});
...
describe("trackLocation", () => {
  it("hands the window to the tab that owns where it went", () => {
    ...
    expect(show(state)).toBe("*code git sessions");
...
describe("closeTab", () => {
  it("hands the window to the right-hand neighbour", () => {
    ...
    expect(show(closeTab(state, "b"))).toBe("code git sessions a *c");
  });

  it("falls back to the left at the end of the strip", () => {
    const state = stripOf("a", "b", "c");
    expect(show(closeTab(state, "c"))).toBe("code git sessions a *b");
  });

  it("leaves the active tab alone when another closes", () => {
    const state = stripOf("a", "b", "c");
    expect(show(closeTab(state, "a"))).toBe("code git sessions b *c");
  });
...
describe("moveTab", () => {
  it("drops a tab at the index, sliding the rest out of its way", () => {
    const state = stripOf("a", "b", "c");
    expect(show(moveTab(state, "a", 5))).toBe("code git sessions b *c a");
    expect(show(moveTab(state, "c", 3))).toBe("code git sessions *c a b");
  });

  it("holds sessions behind the pinned tabs, which do not move", () => {
    const state = stripOf("a", "b");
    expect(show(moveTab(state, "b", 0))).toBe("code git sessions *b a");
    expect(moveTab(state, PROJECT_TAB_ID, 5)).toBe(state);
    expect(moveTab(state, GIT_TAB_ID, 5)).toBe(state);
    expect(moveTab(state, SESSIONS_TAB_ID, 5)).toBe(state);
  });

  it("clamps out-of-range targets and no-ops on a non-move", () => {
    const state = stripOf("a", "b", "c");
    expect(show(moveTab(state, "a", 99))).toBe("code git sessions b *c a");
...
describe("nextModeTab", () => {
  it("steps along the ways of working, wrapping round at the last", () => {
    ...
    expect(nextModeTab(tabs, PROJECT_TAB_ID)?.id).toBe(GIT_TAB_ID);
    expect(nextModeTab(tabs, GIT_TAB_ID)?.id).toBe(SESSIONS_TAB_ID);
