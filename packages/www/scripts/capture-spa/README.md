...
Points a headless Chrome at the reviewer SPA running locally, captures the live
...
- `--state <file.json>` — localStorage entries seeded before first paint.
  reviewer keeps its pane layout, widths and tabs in `reviewer-ui`, so this is how
