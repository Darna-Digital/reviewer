...
The homepage renders the real reviewer interface rather than screenshots:
`scripts/capture-spa` points a headless Chrome at the SPA running on :41812,
captures the live DOM and the CSS that applies to it, and writes the JSON in
`public/spa-snapshots/` that `<SpaSnapshot>` mounts inside a shadow root. Run
`pnpm dev` in the repo root first, then `pnpm capture:home`; see
`scripts/capture-spa/README.md`.
...
`main` that touches this package, with `CLOUDFLARE_API_TOKEN` and
`CLOUDFLARE_ACCOUNT_ID` from the repository's secrets.

## Styling

[Tailwind CSS](https://tailwindcss.com/), configured from `src/styles.css`.
